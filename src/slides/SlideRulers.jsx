  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1 }}>
      ...
      <svg viewBox="0 0 760 680" style={{ width: '100%', maxWidth: 800, flex: 1 }}>
        ...
        {/* Side labels at top */}
        <text x={cx - 50} y={30} textAnchor="end" fontSize="20" fill="var(--text-muted)" fontStyle="italic">Moon's side</text>
        <text x={cx + 50} y={30} textAnchor="start" fontSize="20" fill="var(--text-muted)" fontStyle="italic">Sun's side</text>
          ...
          return (
            <g key={`pair-${i}`}>
              ...
              <text x={midX} y={midY} textAnchor="middle" dominantBaseline="central"
                fontSize="16" fill={pair.color} fontWeight="500" opacity="0.7">
                {pair.ruler}
              </text>
            </g>
          )
        ...
        {signData.map((s, i) => {
          ...
          return (
            <g key={i}>
              <circle cx={x} cy={y} r={36} fill="var(--bg-light)" stroke={s.rColor} strokeWidth="1.5" />
              <text x={x} y={y - 6} textAnchor="middle" dominantBaseline="central" fontSize="28" fill="var(--text)">{s.sym}</text>
              <text x={x} y={y + 16} textAnchor="middle" fontSize="13" fill="var(--text-muted)">{s.name}</text>
              {/* Ruler label outside */}
              <text x={lx} y={ly} textAnchor="middle" dominantBaseline="central" fontSize="16" fill={s.rColor}>{s.ruler}</text>
            </g>
          )
        })}
      </svg>
