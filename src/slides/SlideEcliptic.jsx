    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 1, width: '100%', gap: 40 }}>
        ...
        <svg viewBox="0 0 700 700" style={{ flex: 1, maxHeight: '70vh' }}>
          ...
          {Array.from({ length: 12 }).map((_, i) => {
            ...
            return (
              <g key={i}>
                ...
                <text x={nx} y={ny} textAnchor="middle" dominantBaseline="central" fontSize="14" fill="var(--text-muted)">{names[i]}</text>
              </g>
            )
          })}
          ...
          <text x={cx} y={cy - 8} textAnchor="middle" fontSize="18" fill="var(--text-muted)">Planets travel</text>
          <text x={cx} y={cy + 16} textAnchor="middle" fontSize="18" fill="var(--text-muted)">through this belt</text>
        </svg>
      </div>
    </div>
  )
